...
function startGame(snakeColor) {
  ...
  gameLoop = setInterval(() => {
    ...
    if (
      ...
      saveScore('Nombre del Jugador', totalUnitsConsumed); // Guardar la puntuación
      return;
    }
...
// Función para obtener y mostrar las mejores puntuaciones
...
function displayTopScores(scores) {
  const topScoresElement = $('#top-scores');
  topScoresElement.empty();
  scores.forEach(score => {
    const scoreElement = $('<div>').text(`${score.name}: ${score.score}`);
    topScoresElement.append(scoreElement);
  });
}

// Llamar a fetchTopScores cuando cargue la página o en un momento apropiado
$(document).ready(() => {
  fetchTopScores();
});
...
async function saveScore(name, score) {
  try {
    ...
    fetchTopScores(); // Actualizar las mejores puntuaciones después de guardar la nueva puntuación
  } catch (error) {
    console.error('Error saving score:', error);
  }
}

// Integrar la función saveScore en la lógica de tu juego
function gameOver() {
  clearInterval(gameLoop); // Detener el bucle de juego
  alert('¡Game Over! La serpiente chocó consigo misma o con el borde.'); // Mostrar mensaje de fin de partida
  saveScore('Nombre del Jugador', totalUnitsConsumed); // Guardar la puntuación
...
// No olvides llamar a saveScore cuando el juego termine y quieras guardar la puntuación
